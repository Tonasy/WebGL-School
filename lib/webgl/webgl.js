...
export class WebGLUtility {
  ...
  static createVBO(gl, vertexArray) {
    ...
    // STATIC_DRAW->もうvboの中身は書き換えませんという意味。
    // 書き換える指定も可能だが、CPUの負荷がかかるので普通は書き換えない。（シェーダー側で変更するべき）
   ...
   * @param {number} unitNum - テクスチャユニット番号
   * @return {WebGLTexture}
   */
  static createTexture(gl, resource, unitNum){
    // テクスチャオブジェクトを生成
